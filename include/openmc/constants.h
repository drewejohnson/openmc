...
enum class AngularDistribution : int {
  ANGLE_ISOTROPIC=1,
  ANGLE_32_EQUI,
  ANGLE_TABULAR,
  ANGLE_LEGENDRE,
...
enum class TemperatureTreatment : int {
  TEMPERATURE_NEAREST=1,
  TEMPERATURE_INTERPOLATION,
};
// Reaction types
enum class ReactionTypes : int {
  REACTION_NONE=0,
  TOTAL_XS=1,
  ELASTIC=2,
  N_NONELASTIC=3,
...
enum class PtableParameters : int {
  URR_CUM_PROB, URR_TOTAL, URR_ELASTIC, URR_FISSION, URR_N_GAMMA, URR_HEATING,
};
...
// Electron treatments
enum class ElectronModes : int {
  ELECTRON_LED=1, // Local Energy Deposition
  ELECTRON_TTB, // Thick Target Bremsstrahlung
};
// ============================================================================
// MULTIGROUP RELATED

// MGXS Table Types
enum class MgxsTableTypes : int {
  MGXS_ISOTROPIC=1, // Isotroically weighted data
  MGXS_ANGLE,     // Data by angular bins
};
...
enum class TallyResultEntries : int {
  RESULT_VALUE,
  RESULT_SUM,
  RESULT_SUM_SQ,
};

enum class TallyTypes : int {
  TALLY_VOLUME=1,
  TALLY_MESH_SURFACE,
  TALLY_SURFACE,
};
...
enum class TallyEstimatorTypes : int {
  ESTIMATOR_ANALOG=1,
  ESTIMATOR_TRACKLENGTH,
  ESTIMATOR_COLLISION,
};
...
enum class TallyScoreTypes : int {
  SCORE_FLUX=-1, // flux
...
enum class TallyFilterTypes : int {
  FILTER_UNIVERSE=1,
  FILTER_MATERIAL,
  FILTER_CELL,
};
...
enum class TallySurfaceDirections : int {
  OUT_LEFT=1,  // x min
  IN_LEFT,
  OUT_RIGHT,
  IN_RIGHT,
